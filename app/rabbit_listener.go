...
func NewListener(config *model.RabbitConfig, f HandleFunc, errChan chan errors.AppError) (*RabbitListener, errors.AppError) {
	if config == nil {
		return nil, errors.NewInternalError("rabbit1.listener.new_rabit_listener.arguments_check.config_nil", "rabbit1 config is nil")
	}
...
func (l *RabbitListener) Start() {
	...
	if err != nil {
		l.exit <- errors.NewInternalError("rabbit1.listener.listen.server_connect.fail", err.Error())
		return
	}
	l.connection = conn
	defer conn.Close()
	...
	if err != nil {
		l.exit <- errors.NewInternalError("rabbit1.listener.listen.channel_connect.fail", err.Error())
		return
	}
	defer channel.Close()
	...
	if err != nil {
		l.exit <- errors.NewInternalError("rabbit1.listener.listen.exchange_declare.fail", err.Error())
		return
	}
	...
	if err != nil {
		l.exit <- errors.NewInternalError("rabbit1.listener.listen.queue_declare.fail", err.Error())
		return
	}
	...
	if err != nil {
		l.exit <- errors.NewInternalError("rabbit1.listener.listen.queue_bind.fail", err.Error())
		return
	}
	...
	if err != nil {
		l.exit <- errors.NewInternalError("rabbit1.listener.listen.start_consuming.fail", err.Error())
		return
	}
		...
		for message = range delivery {
			...
			if appErr != nil {
				if checkNoRows(appErr) {
					...
					if err != nil {
						appErr = errors.NewInternalError("rabbit1.listener.listen.acknowledge.fail", err.Error())
					}
				} else {
